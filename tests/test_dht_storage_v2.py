...
def test_localstorage_nested():
    ...
    for subkey, (subvalue, subexpiration, created_at) in d2.items():
    ...
    stored = d1.get(DHTID.generate("foo"))[0].get("subkey1")
    value, expiration_time, created_at = stored
    assert value == b"value1"
    assert expiration_time == time + 2
    ...
    stored = d1.get(DHTID.generate("foo"))[0].get("subkey2")
    value, expiration_time, created_at = stored
    assert value == b"value2"
    assert expiration_time == time + 3
    assert abs(created_at - time) < 0.5  # allow small drift

    assert d1.store(DHTID.generate("foo"), b"nothing", time + 5) is True  # new value has better expiraiton
    stored = d1.get(DHTID.generate("foo"))
    value, expiration_time, created_at = stored
    assert value == b"nothing"

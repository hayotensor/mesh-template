...
def test_get_store(n_peers=10):
    ...
    assert node1.store("key1", "value1", expiration_time=subnet.get_dht_time() + 30)
    ...
    assert node2.store("key1", 123, expiration_time=subnet.get_dht_time() + 31)
    assert node2.store("key2", 456, expiration_time=subnet.get_dht_time() + 32)
    assert node1.get("key1", latest=True).value == 123
    assert node1.get("key2").value == 456

    assert node1.store("key2", subkey="subkey1", value=789, expiration_time=subnet.get_dht_time() + 32)
    assert node2.store("key2", subkey="subkey2", value="pew", expiration_time=subnet.get_dht_time() + 32)
...
async def test_dht_get_visible_maddrs():
    ...
    p2p = await subnet.p2p.P2P.create(announce_maddrs=[dummy_endpoint])
    dht = subnet.DHT(start=True, p2p=p2p)
...
async def test_startup_error():
    with pytest.raises(subnet.p2p.P2PDaemonError, match=r"(?i)Failed to connect to bootstrap peers"):
        subnet.DHT(
            initial_peers=[f"/ip4/127.0.0.1/tcp/{get_free_port()}/p2p/QmdaK4LUeQaKhqSFPRu9N7MvXUEWDxWwtCvPrS444tCgd1"],
            start=True,
        )

    dht = subnet.DHT(start=True, await_ready=False)

...
def _initialize_if_necessary():
    ...
    with _init_lock:
        ...
        _enable_default_handler("mesh")


def get_logger(name: Optional[str] = None) -> logging.Logger:
    """
    Same as ``logging.getLogger()`` but ensures that the default mesh log handler is initialized.

    :note: By default, the mesh log handler (that reads the ``MESH_LOGLEVEL`` env variable and uses
           the colored log formatter) is only applied to messages logged inside the mesh package.
...
def use_mesh_log_handler(where: Union[HandlerMode, str]) -> None:
    """
    Choose loggers where the default mesh log handler is applied. Options for the ``where`` argument are:

    * "in_mesh" (default): Use the mesh log handler in the loggers of the ``mesh`` package.
                               Don't propagate their messages to the root logger.
    * "nowhere": Don't use the mesh log handler anywhere.
                 Propagate the ``mesh`` messages to the root logger.
    * "in_root_logger": Use the mesh log handler in the root logger

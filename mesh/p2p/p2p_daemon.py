...
import mesh.mesh_cli as cli
import mesh.p2p.p2p_daemon_bindings.p2pclient as p2pclient
from mesh.p2p.p2p_daemon_bindings.control import DEFAULT_MAX_MSG_SIZE, P2PDaemonError, P2PHandlerError
from mesh.p2p.p2p_daemon_bindings.datastructures import PeerID, PeerInfo, StreamInfo
from mesh.p2p.p2p_daemon_bindings.utils import ControlFailure
from mesh.proto import crypto_pb2
from mesh.proto.p2pd_pb2 import RPCError
from mesh.utils.asyncio import as_aiter, asingle, cancel_task_if_running
from mesh.utils.crypto import Ed25519PrivateKey, RSAPrivateKey
from mesh.utils.logging import get_logger, golog_level_to_python, loglevel, python_level_to_golog
...
class P2P:
    ...
    _UNIX_SOCKET_PREFIX = "/unix/tmp/mesh-"
    ...
    async def create(
        ...
        *,
        ...
        if not all(arg is None for arg in [quic, use_relay_hop, use_relay_discovery]):
            warnings.warn(
                "Parameters `quic`, `use_relay_hop`, and `use_relay_discovery` of mesh.P2P "
                "have no effect since libp2p 0.17.0 and will be removed in mesh 1.2.0+",
                DeprecationWarning,
                stacklevel=2,
            )

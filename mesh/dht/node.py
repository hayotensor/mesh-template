...
class DHTNode:
    ...
    :note: DHT is optimized to store a lot of temporary metadata that is regularly updated.
     For example, expert heartbeat emitted by a mesh.moe.Server responsible for that expert.
    ...
    Similar to Kademlia RPC protocol, mesh DHT has 3 RPCs:
    ...
    * find - request one or several keys, get values and expiration (if peer finds it locally) and :bucket_size: of
    ...
    async def create(
        ...
        **kwargs,
        ...
        """
        :param p2p: instance of mesh.p2p.P2P that will be used for communication.
          If None, DHTNode will create and manage its own P2P instance with given initial_peers and
          parameters from ``kwargs``
        :param node_id: current node's DHTID for mesh.dht, determines which keys it will store locally,
        ...
        :param kwargs: extra parameters for an internally created instance of mesh.p2p.P2P.
        ...
        if p2p is None:
            ...
            if kwargs:
                raise ValueError(
                    f"**kwargs in DHTNode.create() should be empty if mesh.p2p.P2P instance is provided"

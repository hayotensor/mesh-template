...
import subnet.p2p.p2p_daemon_bindings.p2pclient as p2pclient
import subnet.subnet_cli as cli
from subnet.p2p.p2p_daemon_bindings.control import DEFAULT_MAX_MSG_SIZE, P2PDaemonError, P2PHandlerError
from subnet.p2p.p2p_daemon_bindings.datastructures import PeerID, PeerInfo, StreamInfo
from subnet.p2p.p2p_daemon_bindings.utils import ControlFailure
from subnet.proto import crypto_pb2
from subnet.proto.p2pd_pb2 import RPCError
from subnet.utils.asyncio import as_aiter, asingle, cancel_task_if_running
from subnet.utils.crypto import Ed25519PrivateKey, RSAPrivateKey
from subnet.utils.logging import get_logger, golog_level_to_python, loglevel, python_level_to_golog
...
class P2P:
    ...
    _UNIX_SOCKET_PREFIX = "/unix/tmp/subnet-"
    ...
    async def create(
        ...
        *,
        ...
        if not all(arg is None for arg in [quic, use_relay_hop, use_relay_discovery]):
            warnings.warn(
                "Parameters `quic`, `use_relay_hop`, and `use_relay_discovery` of subnet.P2P "
                "have no effect since libp2p 0.17.0 and will be removed in subnet 1.2.0+",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    @staticmethod
    def generate_rsa_identity(identity_path: str) -> None:
    ...
    @staticmethod
    def generate_ed25519_identity(identity_path: str) -> None:

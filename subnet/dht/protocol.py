...
from subnet.dht.routing import DHTID, BinaryDHTValue, RoutingTable, Subkey
from subnet.dht.storage import DHTLocalStorage, DictionaryDHTValue
...
from subnet.utils import MSGPackSerializer, get_logger
from subnet.utils.authorizers.auth import AuthorizerBase, AuthRole, AuthRPCWrapper
...
class DHTProtocol(ServicerBase):
    ...
    serializer = (
        MSGPackSerializer  # used to pack/unpack DHT Values for transfer over network
    )
    ...
    async def create(
        ...
        self.p2p = p2p
        self.node_id, self.bucket_size, self.num_replicas = (
            node_id,
            bucket_size,
            num_replicas,
        )
        self.wait_timeout = wait_timeout
        self.storage, self.cache = (
            DHTLocalStorage(),
            DHTLocalStorage(maxsize=cache_size),
        )
        ...
        if not self.client_mode:
            await self.add_p2p_handlers(
                self.p2p, AuthRPCWrapper(self, AuthRole.SERVICER, self.authorizer)
            )
            # await self.add_p2p_handlers(self.p2p, AuthRPCWrapper(self, AuthRole.SERVICER, self.authorizer, service_public_key=self.authorizer._local_public_key))
    ...
    def __init__(self, *, _initialized_with_create=False):
        ...
        assert _initialized_with_create, (
            "Please use DHTProtocol.create coroutine to spawn new protocol instances"
        )
        super().__init__()
    ...
    async def call_ping(
        ...
        try:
            async with self.rpc_semaphore:
                ping_request = dht_pb2.PingRequest(
                    peer=self.node_info, validate=validate
                )
                time_requested = get_dht_time()
                response = await self.get_stub(peer).rpc_ping(
        ...
        if responded and validate:
            try:
                if not self.client_mode and response.available:
                    # Debuging libp2p purposes only
                    logger.debug(f"Peer {peer} can access this node")
                ...
                if (
                    response.dht_time
                    != dht_pb2.PingResponse.DESCRIPTOR.fields_by_name[
                        "dht_time"
                    ].default_value
                ):
                    if (
                        response.dht_time
                        < time_requested - MAX_DHT_TIME_DISCREPANCY_SECONDS
                        or response.dht_time
                        > time_responded + MAX_DHT_TIME_DISCREPANCY_SECONDS
                    ):
                        raise ValidationError(
    ...
    async def rpc_ping(
        ...
        response = dht_pb2.PingResponse(
            peer=self.node_info, dht_time=get_dht_time(), available=False
        )
        ...
        if request.peer and request.peer.node_id:
            ...
            if request.validate:
                response.available = (
                    await self.call_ping(sender_peer_id, validate=False) == sender_id
                )
    ...
    async def call_store(
        ...
        in_cache = (
            in_cache if in_cache is not None else [False] * len(keys)
        )  # default value (None)
        in_cache = (
            [in_cache] * len(keys) if isinstance(in_cache, bool) else in_cache
        )  # single bool
        ...
        assert len(keys) == len(values) == len(expiration_time) == len(in_cache), (
            "Data is not aligned"
        )
        ...
        try:
            async with self.rpc_semaphore:
                response = await self.get_stub(peer).rpc_store(
            ...
            asyncio.create_task(
                self.update_routing_table(
                    self.routing_table.get(peer_id=peer), peer, responded=False
                )
            )
            return None

    async def rpc_store(
        self, request: dht_pb2.StoreRequest, context: P2PContext
    ) -> dht_pb2.StoreResponse:
        ...
        assert (
            len(request.keys)
            == len(request.values)
            == len(request.expiration_time)
            == len(request.in_cache)
        )
        ...
        for key, tag, value_bytes, expiration_time, in_cache in zip(
            ...
            if (
                tag == self.IS_DICTIONARY
            ):  # store an entire dictionary with several subkeys
                ...
                if not self._validate_dictionary(
                    key, value_dictionary, DHTRecordRequestType.POST
                ):
                    response.store_ok.append(False)
                    continue

                logger.debug(f"Storing dictionary key_id {key_id}")
                logger.debug(f"Storing dictionary key {key}")
                logger.debug(f"Storing dictionary tag {tag}")
                logger.debug(f"Storing dictionary value_bytes {value_bytes}")
                logger.debug(f"Storing dictionary expiration_time {expiration_time}")
                logger.debug(f"Storing dictionary in_cache {in_cache}")

                response.store_ok.append(
                    all(
                        storage.store_subkey(
                            key_id, subkey, item.value, item.expiration_time
                        )
            ...
            if not self._validate_record(
                key, tag, value_bytes, expiration_time, DHTRecordRequestType.POST
            ):
                response.store_ok.append(False)
                continue
            ...
            if tag == self.IS_REGULAR_VALUE:  # store normal value without subkeys
                ...
                response.store_ok.append(
                    storage.store_subkey(key_id, subkey, value_bytes, expiration_time)
                )
        return response
    ...
    async def call_find(
        ...
        """
        keys = list(keys)
        find_request = dht_pb2.FindRequest(
            keys=list(map(DHTID.to_bytes, keys)), peer=self.node_info
        )
        try:
            async with self.rpc_semaphore:
                logger.debug(f"Calling find on peer {peer}")
                response = await self.get_stub(peer).rpc_find(
            ...
            for key, result in zip(keys, response.results):
                ...
                if result.type == dht_pb2.NOT_FOUND:
                    ...
                    if not self._validate_record(
                        key_bytes,
                        self.IS_REGULAR_VALUE,
                        result.value,
                        result.expiration_time,
                        DHTRecordRequestType.GET,
                    ):
                        output[key] = None, nearest
                        continue

                    output[key] = (
                        ValueWithExpiration(result.value, result.expiration_time),
                        nearest,
                    )
                elif result.type == dht_pb2.FOUND_DICTIONARY:
                    value_dictionary = self.serializer.loads(result.value)
                    if not self._validate_dictionary(
                        key_bytes, value_dictionary, DHTRecordRequestType.GET
                    ):
                        output[key] = None, nearest
                        continue

                    output[key] = (
                        ValueWithExpiration(value_dictionary, result.expiration_time),
                        nearest,
                    )
                else:
    ...
    async def rpc_find(
        self, request: dht_pb2.FindRequest, context: P2PContext
    ) -> dht_pb2.FindResponse:
        """
    ...
    async def update_routing_table(
        self, node_id: Optional[DHTID], peer_id: PeerID, responded=True
    ):
        """
        ...
        """
        node_id = (
            node_id if node_id is not None else self.routing_table.get(peer_id=peer_id)
        )
        if responded:  # incoming request or outgoing request with response
            if node_id not in self.routing_table:
                ...
                for key, item in list(self.storage.items()):
                    ...
                    if neighbors:
                        ...
                        new_node_should_store = (
                            node_id.xor_distance(key) < farthest_distance
                        )
                        this_node_is_responsible = (
                            self.node_id.xor_distance(key) < nearest_distance
                        )
                    if not neighbors or (
                        new_node_should_store and this_node_is_responsible
                    ):
                        data_to_send.append((key, item.value, item.expiration_time))
                if data_to_send:
                    asyncio.create_task(
                        self.call_store(peer_id, *zip(*data_to_send), in_cache=False)
                    )
            ...
            if maybe_node_to_ping is not None:
                ...
                asyncio.create_task(
                    self.call_ping(maybe_node_to_ping[1])
                )  # [1]-th element is that node's peer_id
    ...
    def _validate_dictionary(
        ...
        with dictionary.freeze():
            for subkey, (value_bytes, expiration_time) in dictionary.items():
                ...
                if not self.record_validator.validate(record, type):
                    return False
        return True

...
from subnet.dht.node import DEFAULT_NUM_WORKERS, DHTNode
from subnet.dht.routing import DHTID, DHTKey, DHTValue, Subkey
from subnet.dht.validation import CompositeValidator, RecordValidatorBase
from subnet.p2p import P2P, PeerID
from subnet.utils import MPFuture, switch_to_uvloop
from subnet.utils.logging import configure_subprocess_logging, get_logger
from subnet.utils.multiaddr import Multiaddr
from subnet.utils.timed_storage import DHTExpiration, ValueWithExpiration
...
class DHT(mp.context.ForkProcess):
    """
    A high-level interface to a subnet DHT that runs a single DHT node in a background process.
    * subnet servers periodically announce their experts via declare_experts (dht_handler.py)
    * trainers find most suitable experts via RemoteMixtureOfExperts (beam_search.py)
    ...
    :param kwargs: any other params will be forwarded to DHTNode and subnet.p2p.P2P upon creation
    """
    ...
    def store(
        ...
        **kwargs,
        ...
        :param expiration_time: absolute time when the entry should expire, based on subnet.get_dht_time()
    ...
    def node_id(self) -> DHTID:
        if self._node_id is None:
            if os.getpid() == self.pid:
                ...
                # note: we cannot run_coroutine from the same pid because it would deadlock the event loop
                self._node_id = self.run_coroutine(DHT._get_node_id)
        return self._node_id

    @staticmethod
    async def _get_node_id(_dht: DHT, node: DHTNode) -> DHTID:
        return node.node_id

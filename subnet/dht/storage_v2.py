...
from subnet.dht.routing import DHTID, BinaryDHTValue, Subkey
from subnet.utils.serializer_v2 import MSGPackSerializer
from subnet.utils.timed_storage_v2 import DHTExpiration, KeyType, TimedStorage, ValueType, get_dht_time
...
class DictionaryDHTValue(TimedStorage[Subkey, BinaryDHTValue]):
    ...
    def packb(self) -> bytes:
        ...
        packed_items = [
            [key, value, expiration_time, created_at] for key, (value, expiration_time, created_at) in self.items()
        ]
        return MSGPackSerializer.dumps(
            [self.maxsize, self.latest_expiration_time, self.latest_created_at, packed_items]
        )
...
class DHTLocalStorage(TimedStorage[DHTID, Union[BinaryDHTValue, DictionaryDHTValue]]):
    ...
    def store_subkey(self, key: DHTID, subkey: Subkey, value: BinaryDHTValue, expiration_time: DHTExpiration) -> bool:
        ...
        """
        previous_value, previous_expiration_time, previous_created_at = self.get(key) or (
            b"",
            -float("inf"),
            -float("inf"),
        )

...
def block_ip(ip_address):
    ...
    try:
        ...
        result = subprocess.run(
            ["sudo", "ufw", "deny", "from", ip_address], capture_output=True, text=True, check=True
        )
...
def unblock_ip(ip_address):
    ...
    try:
        ...
        check_result = subprocess.run(
            ["sudo", "ufw", "status", "numbered"], capture_output=True, text=True, check=True
        )
        ...
        # Delete the rule
        result = subprocess.run(
            ["sudo", "ufw", "delete", "deny", "from", ip_address], capture_output=True, text=True, check=True
        )

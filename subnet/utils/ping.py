...
async def ping(
    peer_id: subnet.PeerID,
    _dht: subnet.DHT,
    node: subnet.dht.DHTNode,
    *,
...
async def ping_test_parallel(peer_ids: Sequence[subnet.PeerID], *args, **kwargs) -> Dict[subnet.PeerID, float]:
    rpc_infos = await asyncio.gather(*[ping_test(peer_id, *args, **kwargs) for peer_id in peer_ids])
    return dict(zip(peer_ids, rpc_infos))


async def ping_parallel(peer_ids: Sequence[subnet.PeerID], *args, **kwargs) -> Dict[subnet.PeerID, float]:
    rpc_infos = await asyncio.gather(*[ping(peer_id, *args, **kwargs) for peer_id in peer_ids])
    return dict(zip(peer_ids, rpc_infos))


class PingAggregator:
    def __init__(self, dht: subnet.DHT, *, ema_alpha: float = 0.2, expiration: float = 300):
        self.dht = dht
        self.ema_alpha = ema_alpha
        self.expiration = expiration
        self.ping_emas = subnet.TimedStorage()
        self.lock = threading.Lock()

    def ping(self, peer_ids: Sequence[subnet.PeerID], **kwargs) -> None:
        current_rtts = self.dht.run_coroutine(partial(ping_parallel, peer_ids, **kwargs))
        logger.debug(f"Current RTTs: {current_rtts}")

        with self.lock:
            expiration = subnet.get_dht_time() + self.expiration
    ...
    def to_dict(self) -> Dict[subnet.PeerID, float]:

...
info_cache = subnet.TimedStorage()
...
async def check_reachability(peer_id, _, node, *, fetch_info=False, connect_timeout=5, expiration=300, use_cache=True):
    ...
    try:
        async with timeout(connect_timeout):
            if fetch_info:
                ...
                response = await stub.rpc_info(subnet.proto.runtime_pb2.Empty())
                rpc_info = subnet.MSGPackSerializer.loads(response.serialized_info)
                rpc_info["ok"] = True
            else:
                # For Bootstrap
    ...
    info_cache.store(peer_id, rpc_info, subnet.get_dht_time() + expiration)
    return rpc_info

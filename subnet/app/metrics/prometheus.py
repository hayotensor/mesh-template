...
from subnet.app.metrics.config import INITIAL_PEERS, UPDATE_PERIOD
from subnet.app.utils.consensus import OnChainConsensusScore
from subnet.substrate.chain_functions import Hypertensor
from subnet.utils.data_structures import RemoteModuleInfo, ServerState
from subnet.utils.dht import get_node_infos
from subnet.utils.multiaddr import Multiaddr
from subnet.utils.p2p_utils import check_reachability_parallel, extract_peer_ip_info, get_peers_ips


class MetricsServer:
    def __init__(self, hypertensor: Hypertensor, dht: DHT, port: int = 8000):
    ...
    def get_heartbeat_metrics(self) -> int:
        ...
        bootstrap_states = [
            "online" if reach_infos[peer_id]["ok"] else "unreachable" for peer_id in bootstrap_peer_ids
        ]
        ...
        peers_info = {
            ...
            for peer in self.dht.run_coroutine(get_peers_ips)
        }
    ...
    def normalize_scores(self, scores: Dict[str, float], target_total: float) -> Dict[str, float]:
        ...
        return {peer_id: (score / total) * target_total for peer_id, score in scores.items()}

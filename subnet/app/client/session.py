...
from subnet import deserialize_torch_tensor, get_logger, serialize_torch_tensor
...
from subnet.proto import inference_protocol_pb2
        ...
        for attempt_no in itertools.count():
            try:
                ...
                stub = MockProtocol.get_server_stub(
                    self._remote_manager.state.p2p, server_session.peer_id, self.authorizer
                )
                ...
                self._remote_manager.on_request_failure(server_session.peer_id if server_session is not None else None)

...
from subnet import get_dht_time, get_logger
from subnet.dht.routing import DHTID
from subnet.dht.validation import DHTRecord, DHTRecordRequestType
from subnet.substrate.chain_functions import EpochData, Hypertensor
from subnet.substrate.config import BLOCK_SECS, EPOCH_LENGTH
from subnet.utils.key import extract_peer_id_from_record_validator
...
class MockHypertensorCommitReveal:
    ...
    def _has_exceeded_store_limit(
        ...
        if count >= limit:
            logger.debug(
                f"Peer {peer_id} exceeded store limit for {key_type} (epoch {epoch}, count={count}, limit={limit})"
            )
            return True
        return False

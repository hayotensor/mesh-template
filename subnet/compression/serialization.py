...
from subnet.compression.base import CompressionBase, CompressionInfo, NoCompression
...
from subnet.proto import runtime_pb2
from subnet.utils.streaming import combine_from_streaming
...
def deserialize_past_key_values(
    tensors: List[runtime_pb2.Tensor], num_layers: int
) -> Tuple[Tuple[torch.Tensor, torch.Tensor], ...]:
    flat_tensors = [deserialize_torch_tensor(t) for t in tensors]
    return tuple((flat_tensors[i], flat_tensors[i + 1]) for i in range(0, num_layers * 2, 2))

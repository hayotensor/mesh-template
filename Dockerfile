...
COPY requirements.txt subnet/requirements.txt
COPY requirements-dev.txt subnet/requirements-dev.txt
RUN pip install --no-cache-dir -r subnet/requirements.txt && \
  pip install --no-cache-dir -r subnet/requirements-dev.txt && \
  rm -rf ~/.cache/pip
